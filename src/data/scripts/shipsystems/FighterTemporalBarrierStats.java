...
public class FighterTemporalBarrierStats extends BaseShipSystemScript {
	...
	public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
		...
		stats.getTimeMult().modifyFlat(id, 1.5f);
	...
	public StatusData getStatusData(int index, State state, float effectLevel) {
		if (index == 0) {
			return new StatusData("shield damage taken reduced by 30%", false);
		}
		if (index == 1) {
			return new StatusData("shield upkeep reduced by 40%", false);
		}
		if (index == 2) {
			return new StatusData("armor and hull damage taken reduced by 30%", false);
		}
			if (index == 3) {
